...
%HSC
xdot(1) =-(b_HSC*3+a_HSC_MPP*3+a_HSC_mat*3+g_HSC*3)*x1;
...
xdot(8) =a_HSC_mat*3*(x1-x8);
xdot(9) =a_HSC_mat*3*(x8-x9);
xdot(10) =a_HSC_mat*3*(x9-x10);
xdot(11) =g_HSC*3*(x1-x11);
xdot(12) =g_HSC*3*(x11-x12);
xdot(13) =g_HSC*3*(x12-x13);
xdot(14) =+2*b_HSC*3*x4-(b_HSC*3+a_HSC_MPP*3+a_HSC_mat*3+g_HSC*3)*x14;
...
xdot(21) =a_HSC_mat*3*(x14-x21);
xdot(22) =a_HSC_mat*3*(x21-x22);
xdot(23) =a_HSC_mat*3*(x22-x23);
xdot(24) =g_HSC*3*(x14-x24);
xdot(25) =g_HSC*3*(x24-x25);
xdot(26) =g_HSC*3*(x25-x26);
xdot(27) =+2*b_HSC*3*x17-(b_HSC*3+a_HSC_MPP*3+a_HSC_mat*3+g_HSC*3)*x27;
...
xdot(34) =a_HSC_mat*3*(x27-x34);
xdot(35) =a_HSC_mat*3*(x34-x35);
xdot(36) =a_HSC_mat*3*(x35-x36);
xdot(37) =g_HSC*3*(x27-x37);
xdot(38) =g_HSC*3*(x37-x38);
xdot(39) =g_HSC*3*(x38-x39);
xdot(40) =+2*b_HSC*3*x30-(b_HSC*3+a_HSC_MPP*3+a_HSC_mat*3+g_HSC*3)*x40;
...
xdot(47) =a_HSC_mat*3*(x40-x47);
xdot(48) =a_HSC_mat*3*(x47-x48);
xdot(49) =a_HSC_mat*3*(x48-x49);
xdot(50) =g_HSC*3*(x40-x50);
xdot(51) =g_HSC*3*(x50-x51);
xdot(52) =g_HSC*3*(x51-x52);
xdot(53) =+2*b_HSC*3*x43-(b_HSC*3+a_HSC_MPP*3+a_HSC_mat*3+g_HSC*3)*x53;
...
xdot(60) =a_HSC_mat*3*(x53-x60);
xdot(61) =a_HSC_mat*3*(x60-x61);
xdot(62) =a_HSC_mat*3*(x61-x62);
xdot(63) =g_HSC*3*(x53-x63);
xdot(64) =g_HSC*3*(x63-x64);
xdot(65) =g_HSC*3*(x64-x65);
xdot(66) =+2*b_HSC*3*x56-(b_HSC*3+a_HSC_MPP*3+a_HSC_mat*3+g_HSC*3)*x66;
...
xdot(73) =a_HSC_mat*3*(x66-x73);
xdot(74) =a_HSC_mat*3*(x73-x74);
xdot(75) =a_HSC_mat*3*(x74-x75);
xdot(76) =g_HSC*3*(x66-x76);
xdot(77) =g_HSC*3*(x76-x77);
xdot(78) =g_HSC*3*(x77-x78);
xdot(79) =+2*b_HSC*3*x69+2*b_HSC*3*x82-(b_HSC*3+a_HSC_MPP*3+a_HSC_mat*3+g_HSC*3)*x79;
...
xdot(86) =a_HSC_mat*3*(x79-x86);
xdot(87) =a_HSC_mat*3*(x86-x87);
xdot(88) =a_HSC_mat*3*(x87-x88);
...
%MLP
xdot(183) =a_MPP_MLP*3*x101-(b_MLP*3+a_MLP*3)*x183;
...
xdot(190) =a_MPP_MLP*3*x114+2*b_MLP*3*x186-(b_MLP*3+a_MLP*3)*x190;
...
xdot(197) =a_MPP_MLP*3*x127+2*b_MLP*3*x193-(b_MLP*3+a_MLP*3)*x197;
...
xdot(204) =a_MPP_MLP*3*x140+2*b_MLP*3*x200-(b_MLP*3+a_MLP*3)*x204;
...
xdot(211) =a_MPP_MLP*3*x153+2*b_MLP*3*x207-(b_MLP*3+a_MLP*3)*x211;
...
xdot(218) =a_MPP_MLP*3*x166+2*b_MLP*3*x214-(b_MLP*3+a_MLP*3)*x218;
...
xdot(225) =a_MPP_MLP*3*x179+2*b_MLP*3*x221+2*b_MLP*3*x228-(b_MLP*3+a_MLP*3)*x225;
...
%CMP
xdot(232) =a_MPP_CMP*3*x98-(b_CMP*3+a_CMP_MEP*3+a_CMP_GMP*3+g_CMP*3)*x232;
xdot(233) =+b_CMP*3*(x232-x233);
xdot(234) =+b_CMP*3*(x233-x234);
xdot(235) =+b_CMP*3*(x234-x235);
xdot(236) =a_CMP_MEP*3*(x232-x236);
xdot(237) =a_CMP_MEP*3*(x236-x237);
xdot(238) =a_CMP_MEP*3*(x237-x238);
xdot(239) =a_CMP_GMP*3*(x232-x239);
xdot(240) =a_CMP_GMP*3*(x239-x240);
xdot(241) =a_CMP_GMP*3*(x240-x241);
xdot(242) =g_CMP*3*(x232-x242);
xdot(243) =g_CMP*3*(x242-x243);
xdot(244) =g_CMP*3*(x243-x244);
xdot(245) =a_MPP_CMP*3*x111+2*b_CMP*3*x235-(b_CMP*3+a_CMP_MEP*3+a_CMP_GMP*3+g_CMP*3)*x245;
xdot(246) =+b_CMP*3*(x245-x246);
xdot(247) =+b_CMP*3*(x246-x247);
xdot(248) =+b_CMP*3*(x247-x248);
xdot(249) =a_CMP_MEP*3*(x245-x249);
xdot(250) =a_CMP_MEP*3*(x249-x250);
xdot(251) =a_CMP_MEP*3*(x250-x251);
xdot(252) =a_CMP_GMP*3*(x245-x252);
xdot(253) =a_CMP_GMP*3*(x252-x253);
xdot(254) =a_CMP_GMP*3*(x253-x254);
xdot(255) =g_CMP*3*(x245-x255);
xdot(256) =g_CMP*3*(x255-x256);
xdot(257) =g_CMP*3*(x256-x257);
xdot(258) =a_MPP_CMP*3*x124+2*b_CMP*3*x248-(b_CMP*3+a_CMP_MEP*3+a_CMP_GMP*3+g_CMP*3)*x258;
xdot(259) =+b_CMP*3*(x258-x259);
xdot(260) =+b_CMP*3*(x259-x260);
xdot(261) =+b_CMP*3*(x260-x261);
xdot(262) =a_CMP_MEP*3*(x258-x262);
xdot(263) =a_CMP_MEP*3*(x262-x263);
xdot(264) =a_CMP_MEP*3*(x263-x264);
xdot(265) =a_CMP_GMP*3*(x258-x265);
xdot(266) =a_CMP_GMP*3*(x265-x266);
xdot(267) =a_CMP_GMP*3*(x266-x267);
xdot(268) =g_CMP*3*(x258-x268);
xdot(269) =g_CMP*3*(x268-x269);
xdot(270) =g_CMP*3*(x269-x270);
xdot(271) =a_MPP_CMP*3*x137+2*b_CMP*3*x261-(b_CMP*3+a_CMP_MEP*3+a_CMP_GMP*3+g_CMP*3)*x271;
xdot(272) =+b_CMP*3*(x271-x272);
xdot(273) =+b_CMP*3*(x272-x273);
xdot(274) =+b_CMP*3*(x273-x274);
xdot(275) =a_CMP_MEP*3*(x271-x275);
xdot(276) =a_CMP_MEP*3*(x275-x276);
xdot(277) =a_CMP_MEP*3*(x276-x277);
xdot(278) =a_CMP_GMP*3*(x271-x278);
xdot(279) =a_CMP_GMP*3*(x278-x279);
xdot(280) =a_CMP_GMP*3*(x279-x280);
xdot(281) =g_CMP*3*(x271-x281);
xdot(282) =g_CMP*3*(x281-x282);
xdot(283) =g_CMP*3*(x282-x283);
xdot(284) =a_MPP_CMP*3*x150+2*b_CMP*3*x274-(b_CMP*3+a_CMP_MEP*3+a_CMP_GMP*3+g_CMP*3)*x284;
xdot(285) =+b_CMP*3*(x284-x285);
xdot(286) =+b_CMP*3*(x285-x286);
xdot(287) =+b_CMP*3*(x286-x287);
xdot(288) =a_CMP_MEP*3*(x284-x288);
xdot(289) =a_CMP_MEP*3*(x288-x289);
xdot(290) =a_CMP_MEP*3*(x289-x290);
xdot(291) =a_CMP_GMP*3*(x284-x291);
xdot(292) =a_CMP_GMP*3*(x291-x292);
xdot(293) =a_CMP_GMP*3*(x292-x293);
xdot(294) =g_CMP*3*(x284-x294);
xdot(295) =g_CMP*3*(x294-x295);
xdot(296) =g_CMP*3*(x295-x296);
xdot(297) =a_MPP_CMP*3*x163+2*b_CMP*3*x287-(b_CMP*3+a_CMP_MEP*3+a_CMP_GMP*3+g_CMP*3)*x297;
xdot(298) =+b_CMP*3*(x297-x298);
xdot(299) =+b_CMP*3*(x298-x299);
xdot(300) =+b_CMP*3*(x299-x300);
xdot(301) =a_CMP_MEP*3*(x297-x301);
xdot(302) =a_CMP_MEP*3*(x301-x302);
xdot(303) =a_CMP_MEP*3*(x302-x303);
xdot(304) =a_CMP_GMP*3*(x297-x304);
xdot(305) =a_CMP_GMP*3*(x304-x305);
xdot(306) =a_CMP_GMP*3*(x305-x306);
xdot(307) =g_CMP*3*(x297-x307);
xdot(308) =g_CMP*3*(x307-x308);
xdot(309) =g_CMP*3*(x308-x309);
xdot(310) =a_MPP_CMP*3*x176+2*b_CMP*3*x300+2*b_CMP*3*x313-(b_CMP*3+a_CMP_MEP*3+a_CMP_GMP*3+g_CMP*3)*x310;
xdot(311) =+b_CMP*3*(x310-x311);
xdot(312) =+b_CMP*3*(x311-x312);
xdot(313) =+b_CMP*3*(x312-x313);
xdot(314) =a_CMP_MEP*3*(x310-x314);
xdot(315) =a_CMP_MEP*3*(x314-x315);
xdot(316) =a_CMP_MEP*3*(x315-x316);
xdot(317) =a_CMP_GMP*3*(x310-x317);
...
%GMP
xdot(323) =a_CMP_GMP*3*x241-(b_GMP*3+a_GMP_mat*3+g_GMP*3)*x323;
...
xdot(333) =a_CMP_GMP*3*x254+2*b_GMP*3*x326-(b_GMP*3+a_GMP_mat*3+g_GMP*3)*x333;
...
xdot(343) =a_CMP_GMP*3*x267+2*b_GMP*3*x336-(b_GMP*3+a_GMP_mat*3+g_GMP*3)*x343;
...
xdot(353) =a_CMP_GMP*3*x280+2*b_GMP*3*x346-(b_GMP*3+a_GMP_mat*3+g_GMP*3)*x353;
...
xdot(363) =a_CMP_GMP*3*x293+2*b_GMP*3*x356-(b_GMP*3+a_GMP_mat*3+g_GMP*3)*x363;
...
xdot(373) =a_CMP_GMP*3*x306+2*b_GMP*3*x366-(b_GMP*3+a_GMP_mat*3+g_GMP*3)*x373;
...
xdot(383) =a_CMP_GMP*3*x319+2*b_GMP*3*x376+2*b_GMP*3*x386-(b_GMP*3+a_GMP_mat*3+g_GMP*3)*x383;
...
%MEP
xdot(393) =a_CMP_MEP*3*x238-(b_MEP*3+a_MEP_mat*3+g_MEP*3)*x393;
...
xdot(403) =a_CMP_MEP*3*x251+2*b_MEP*3*x396-(b_MEP*3+a_MEP_mat*3+g_MEP*3)*x403;
...
xdot(413) =a_CMP_MEP*3*x264+2*b_MEP*3*x406-(b_MEP*3+a_MEP_mat*3+g_MEP*3)*x413;
...
xdot(423) =a_CMP_MEP*3*x277+2*b_MEP*3*x416-(b_MEP*3+a_MEP_mat*3+g_MEP*3)*x423;
...
xdot(433) =a_CMP_MEP*3*x290+2*b_MEP*3*x426-(b_MEP*3+a_MEP_mat*3+g_MEP*3)*x433;
...
xdot(443) =a_CMP_MEP*3*x303+2*b_MEP*3*x436-(b_MEP*3+a_MEP_mat*3+g_MEP*3)*x443;
...
xdot(453) =a_CMP_MEP*3*x316+2*b_MEP*3*x446+2*b_MEP*3*x456-(b_MEP*3+a_MEP_mat*3+g_MEP*3)*x453;
...
%mat
xdot(463) =a_HSC_mat*3*x10+a_MEP_mat*3*x399+a_GMP_mat*3*x329-(b_mat*3+g_mat*3)*x463;
...
xdot(470) =a_HSC_mat*3*x23+a_MEP_mat*3*x409+a_GMP_mat*3*x339+2*b_mat*3*x466-(b_mat*3+g_mat*3)*x470;
...
xdot(477) =a_HSC_mat*3*x36+a_MEP_mat*3*x419+a_GMP_mat*3*x349+2*b_mat*3*x473-(b_mat*3+g_mat*3)*x477;
...
xdot(484) =a_HSC_mat*3*x49+a_MEP_mat*3*x429+a_GMP_mat*3*x359+2*b_mat*3*x480-(b_mat*3+g_mat*3)*x484;
...
xdot(491) =a_HSC_mat*3*x62+a_MEP_mat*3*x439+a_GMP_mat*3*x369+2*b_mat*3*x487-(b_mat*3+g_mat*3)*x491;
...
xdot(498) =a_HSC_mat*3*x75+a_MEP_mat*3*x449+a_GMP_mat*3*x379+2*b_mat*3*x494-(b_mat*3+g_mat*3)*x498;
...
xdot(505) =a_HSC_mat*3*x88+a_MEP_mat*3*x459+a_GMP_mat*3*x389+2*b_mat*3*x501+2*b_mat*3*x508-(b_mat*3+g_mat*3)*x505;
...
% INITIAL CONDITIONS
x0 = sym(zeros(size(x)));
x0([1,92,183,232,323,393,463])=exp(p(1:7))-1;
