function [] = lineage_hierarchies_main()
...
%       processes
% function compares models of different lineage hierarchies 
...
[opt,model_str,j_end] = getLineageHierarchySettings();
...
for j=1:j_end %index of model hierarchy used for fit
    ...
    %update options:
    [opt,n_states] = updateLineageHierarchySettings(model_str,j,opt);
    ...
    for i_ID=firstIndividual:opt.n_individuals %index for individual for which parameter inference is performed
        ...
        clearvars -except firstIndividual i_ID opt model_str j j_end n_states par_min par_max ...
...
saveModelSelectionResult(opt,AIC_all_mat,AIC_corrected_all_mat,BIC_all_mat,LOGL_all_mat,AIC_mat,AIC_corrected_mat,BIC_mat,LOGL_mat,Time_mat,MS_num_mat)
    ...
    function [opt,n_states] = updateLineageHierarchySettings(model_str,j,opt)
