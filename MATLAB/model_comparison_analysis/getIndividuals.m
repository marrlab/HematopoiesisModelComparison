function [individuals,dataSet] = getIndividuals(fileName,bool_fit_repetitions_seperately)
...
sampleNums = cellfun(@(x) x(regexp(x,'[H]')+1:end), f_str, 'un', 0);
sampleNums_uni = unique(sampleNums,'stable');
% sampleIDs_uni = unique(cellfun(@(x) x(regexp(x,'[H]')+1:end), Fnames_uni, 'un', 0),'stable');
...
if bool_fit_repetitions_seperately
    sampleNames = cellfun(@(x) x(regexp(x,'[H]')+1:end), f_str(find(strwcmp(f_str,'*H*'))), 'un', 0);
    ...
    I=unique(strcat(sampleNames,strcat('_',num2str(DataTable.repetition(find(strwcmp(f_str,'*H*')))))),'stable');
