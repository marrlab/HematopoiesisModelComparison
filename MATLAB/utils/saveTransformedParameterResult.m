function [par_min,par_max,CI_lower,CI_upper,PAR_OPT_T,PAR_TEST_T,logL_vec] = saveTransformedParameterResult(parameters,par_min,par_max,CI_lower,CI_upper,PAR_OPT_T,PAR_TEST_T,transformation_str,opt,theta_test,i_ID,rate_names_test,logL_vec)
    individuals_str = opt.individuals{i_ID};
    LogL_vec(i_ID) = parameters.MS.logPost(1);
    for ts_ID=1:length(transformation_str)
        if ~opt.realdata && strcmp(opt.model_sim,opt.model)
            [par_min(ts_ID,:,i_ID), par_max(ts_ID,:,i_ID), CI_lower(ts_ID,:,i_ID), CI_upper(ts_ID,:,i_ID), PAR_OPT_T(ts_ID,:,i_ID), PAR_TEST_T(ts_ID,:,i_ID), rate_names_opt] = transformParameterResult(parameters,theta_test,opt,transformation_str{ts_ID});
        else
            [par_min(ts_ID,:,i_ID), par_max(ts_ID,:,i_ID), CI_lower(ts_ID,:,i_ID), CI_upper(ts_ID,:,i_ID), PAR_OPT_T(ts_ID,:,i_ID), ~ , rate_names_opt] = transformParameterResult(parameters,[],opt,transformation_str{ts_ID});
        end
    end
