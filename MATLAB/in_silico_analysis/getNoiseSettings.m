function [opt] = getNoiseSettings(opt,noiseLevel)
    ...
    function [sigma] = getRealisticNoiseLevelFromRealDataFitResults(opt)
        ...
        path=['./results_',num2str(opt.n_divStates),'divs_',num2str(opt.n_intermediateStates),'iS_',HO_str,'LogNormal',ic_str];
        ...
        if ~isempty(path) && ~(exist(path, 'dir')==0)
           ...
           error('Realistic noise parameter cannot be loaded! Run lineage_hierarchies_main() for specified settings first or specify a different noise level in getInSilicoSettings.m!') 
        end
    end
